...
from .serializers import empRegisterSerializer
from .models import empUserProfile
from .utils import generateOtp, sendOtp_email
...
class empRegisterView(APIView):
    ...
    def post(self, request):
        serializer = empRegisterSerializer(data=request.data)
        if serializer.is_valid():
            # Save user and profile
            user = serializer.save()
            emp_profile = user.emp_profile
...
class VerifyOTPview(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            emp_profile = empUserProfile.objects.get(user__email=email)

            # Check if already verified
            if emp_profile.otp_verified:
                return Response({"message": "Account is already verified."}, status=status.HTTP_200_OK)

            # Validate OTP
            if emp_profile.otp == otp:
                emp_profile.otp_verified = True
                emp_profile.save()
